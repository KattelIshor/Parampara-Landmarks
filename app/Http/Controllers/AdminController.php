...
public function list_slider(Request $request)
{
    ...
    $data = compact('title', 'menu', 'slider');
    return view('AdminPanel.slider.list', $data);
}
...
public function add_slider(Request $request)
{
    ...
    $data = compact('title', 'menu');
    return view('AdminPanel.slider.form', $data);
}
...
public function slider_added(Request $request)
{
    ...
    $slider->slug_name = str_slug($request->name);
    $image = $request->file('image');
    $iname = date('Ym') . '-' . rand() . '.' . $image->extension();
    $store = $image->storeAs('public/images/slider', $iname);
    if ($store) {
        $slider->image = $iname;
    }
    $slider->save();
...
public function del_slider(Request $request)
{
    $valid = validator($request->route()->parameters(), [
        'id' => 'exists:slider,id'
    ])->validate();
    ...
    if ($valid) {
        ...
        $pro = Property::where('slider', $id)->get();
...
public function slider_edited(Request $request)
{
    $valid = validator($request->route()->parameters(), [
        'id' => 'exists:slider,id'
    ])->validate();
